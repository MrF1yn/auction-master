...
import { connectToDatabaseWithPrisma, disconnectFromDatabaseWithPrisma } from './config/prisma-client.config';
import { connectToRedisServer, disconnectFromRedisServer } from './config/redis-client.config';
import { generalRateLimiter } from './middleware/rate-limiter.middleware';
import { globalErrorHandlerMiddleware, notFoundHandlerMiddleware } from './middleware/global-error-handler.middleware';
...
import { registerBidEventHandlers, setSocketIoServerInstance } from './sockets/bid-events.socket';
...
io.on('connection', (socket) => {
  ...
  socket.on('disconnect', (reason) => {
    logInfoMessage('Socket disconnected', { socketId: socket.id, userId, reason });
  });
...
async function checkExpiredAuctions(): Promise<void> {
  try {
    const endedCount = await markExpiredAuctionsAsEnded();
    if (endedCount > 0) {
      logInfoMessage(`Marked ${endedCount} auctions as ended`);
      io.emit('auctions:ended', { count: endedCount });
    }
  } catch (error) {
...
function startAuctionExpirationChecker(): void {
  ...
  logInfoMessage('Auction expiration checker started', { intervalMs: AUCTION_CHECK_INTERVAL });
}
...
async function startServer(): Promise<void> {
  try {
    ...
    httpServer.listen(port, () => {
      logInfoMessage('Server started', { port, environment: environmentConfig.NODE_ENV });
      console.log(`
========================================

...
// Defines routes for auction items
...
import { HTTP_STATUS_OK, HTTP_STATUS_NOT_FOUND, HTTP_STATUS_CREATED, HTTP_STATUS_BAD_REQUEST } from '../constants/error-codes.constants';

const auctionItemsRouter = Router();

// ==============================|| CREATE AUCTION ITEM ||============================== //

auctionItemsRouter.post('/', requireJwtAuthentication as any, async (request: AuthenticatedRequest, response: Response) => {
  try {
    ...
    const { title, description, startingPrice, minimumBidIncrement, durationInMinutes, imageUrl } = request.body;

    // Validate required fields
    if (!title || !description || startingPrice === undefined || durationInMinutes === undefined) {
      response.status(HTTP_STATUS_BAD_REQUEST).json({
        success: false,
        errorCode: 'VALIDATION_ERROR',
        errorMessage: 'Title, description, starting price, and duration are required'
      });
      return;
    }
    ...
    const result = await createAuctionItem({
      ...
      auctionDurationInMinutes: parseInt(durationInMinutes, 10),
...
auctionItemsRouter.get('/', requireJwtAuthentication as any, async (_request: AuthenticatedRequest, response: Response) => {
  try {
...
auctionItemsRouter.get('/:auctionItemId', requireJwtAuthentication as any, async (request: AuthenticatedRequest, response: Response) => {
  try {

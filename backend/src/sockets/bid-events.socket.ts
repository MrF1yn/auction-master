...
import { processBidWithDistributedLock, fetchCurrentAuctionBidInfo } from '../services/bid-processor.service';
...
export function registerBidEventHandlers(socket: Socket): void {
  socket.on(SOCKET_EVENT_CLIENT_JOIN_AUCTION_ROOM, (payload: JoinAuctionRoomPayload) => handleJoinRoom(socket, payload));
  socket.on(SOCKET_EVENT_CLIENT_LEAVE_AUCTION_ROOM, (payload: LeaveAuctionRoomPayload) => handleLeaveRoom(socket, payload));
  socket.on(SOCKET_EVENT_CLIENT_PLACE_BID, (payload: PlaceBidPayload) => handlePlaceBid(socket, payload));
}

async function handleJoinRoom(socket: Socket, payload: JoinAuctionRoomPayload): Promise<void> {
  ...
  logSocketEvent('JOIN_ROOM', socket.id, userData?.userId, { auctionItemId, roomName });
...
async function handleLeaveRoom(socket: Socket, payload: LeaveAuctionRoomPayload): Promise<void> {
  const userData = getUserDataFromSocket(socket);
  const { auctionItemId } = payload;
  ...
  logSocketEvent('LEAVE_ROOM', socket.id, userData?.userId, { auctionItemId, roomName });
  socket.emit(SOCKET_EVENT_SERVER_LEFT_AUCTION_ROOM, { auctionItemId });
}

async function handlePlaceBid(socket: Socket, payload: PlaceBidPayload): Promise<void> {
  const userData = getUserDataFromSocket(socket);
  ...
  if (!auctionItemId || typeof bidAmountInDollars !== 'number' || bidAmountInDollars <= 0) {
    socket.emit(SOCKET_EVENT_SERVER_BID_PLACED_ERROR, {
      auctionItemId,
  ...
  logSocketEvent('BID_ATTEMPT', socket.id, userData.userId, { auctionItemId, bidAmountInDollars });

  try {
    const result = await processBidWithDistributedLock(auctionItemId, userData.userId, bidAmountInDollars);
    ...
    if (result.wasBidSuccessful) {
      ...
      ioServer?.to(roomName).emit(SOCKET_EVENT_SERVER_BID_UPDATE_BROADCAST, broadcastPayload);
      ...
      logSocketEvent('BID_FAILED', socket.id, userData.userId, { auctionItemId, errorCode: result.errorCode });
    }
  } catch (error) {
    logErrorMessage('Bid processing error', error, { socketId: socket.id, userId: userData.userId, auctionItemId });
    socket.emit(SOCKET_EVENT_SERVER_BID_PLACED_ERROR, {
      auctionItemId,
